...
!define REG_KEY "Software\${COMPANY}\${COMPRESSED_NAME}"

; Uniinstaller registry key, used to register the logging agent
!define STACKDRIVER_LOGGING_UNINST_REG_KEY "${UNINST_REG_KEY}\${DISPLAY_NAME}"

; Directory used to install all files into.  This makes uninstall easier and safer
!define MAIN_INSTDIR "$INSTDIR\sdl-agent"

; Absolute location of the uninstaller
!define UNINSTALLER_LOCATAION "$INSTDIR\uninstall.exe"
...
Section "Install" 
  ...
  WriteUninstaller ${UNINSTALLER_LOCATAION}
  ...
  ${RegisterUninstallSoftware} \
    "${DISPLAY_NAME}" "${UNINSTALLER_LOCATAION}" "${UI_ICON}" "${COMPANY}" "$0"
...
; Verify the install directory is correct.  There is a bug in an gem
...
Section "Uninstall"
  ...
  Delete /REBOOTOK ${UNINSTALLER_LOCATAION}
